...
> **Writing a document whose body reaches a Mission Control Task tab?** Read [`references/markdown-format.md`](references/markdown-format.md) — the rendering contract for MC's markdown pipeline (table column counts, consistent list markers, heading levels, and where raw HTML does/doesn't render), verified against the real renderer. Bodies are pushed verbatim, so the file *is* what the user sees.

> **Contract version: 2.3.** Bump `schema_version` in document frontmatter and update this file together when the contract changes.
>
> **2.3.27 (`/dev:build` §9g + `/dev:commit` Stages 3/4/5 — fix loop / repair loop execution is MANDATORY and VERIFIABLE; downstream gates refuse to advance without invocation trace).** User-reported gap: findings surface at security review (build-time Stage 9 OR commit-time Stage 3), code review (Stage 4), acceptance regression (Stage 5) — and the agent moves on to the next stage without invoking the fix loop / repair loop. Same class as v2.3.25's Stage 7 semantic-merge skip: the spec described the loop, the agent read the spec as documentation, applied any fix inline (or skipped entirely), and pushed broken code with an unaddressed review flag. Fix — same enforcement pattern as v2.3.25: (a) **`stage-6-fix-loop.md` §6a.i (new)** — Stage 6 execution is MANDATORY and VERIFIABLE whenever Stages 3/4/5 have blocking findings; origin stage MUST invoke `dev-stack-adaptive-implementation` in fix-mode via the Skill / Agent tool and record `stage-<n>.fix_loop_invocation:` in `commit-run.md` with `invoked_at`, `subagent_id`, `origin_findings`, `fix_attempts[]`, `broad_reruns_used`, `exit_state`; internal reasoning that "the finding is small, I'll just fix it inline" is a spec violation; (b) **`stage-3-security.md` §3e, `stage-4-code-review.md` §4d, `stage-5-acceptance.md` §5e** — route-to-fix-loop sections now explicitly require the invocation trace to be recorded, not just "jump to Stage 6"; (c) **`stage-8-9-push-pr.md` §8a preconditions (new HARD gates)** — refuses push if pre-loop `stage-3.findings.critical > 0` OR `high > 0` AND no `stage-3.fix_loop_invocation`; same for Stage 4 (Blocker/Major) and Stage 5 (regression/fail); also gates on POST-loop counts being zero, so an ESCALATED fix loop that ended without clearing findings can't push either; (d) **`/dev:build` Stage 9 §9g.i (new)** — same MANDATORY-and-VERIFIABLE contract for the build-time repair loop when Critical findings surface; records `stage-9.repair_loop_invocation:`; (e) **`/dev:build` Stage 10 §10a HARD gates (v2.3.27)** — refuses to flip context units from `designed → implemented` if pre-repair `stage-9.findings.critical > 0` AND no `stage-9.repair_loop_invocation`, or if post-repair critical is still > 0; flipping the graph on code with an unaddressed Critical security finding would misrepresent the graph as "verified" when the review actively said "block". Each halt names a specific `blocker:` code so `/dev:commit --resume` / `/dev:build --resume` can re-run from the gap. Zero-finding runs skip the loop entirely and do NOT need to record an invocation trace — the gate is: pre-loop blocking findings > 0 IMPLIES invocation trace MUST be present.
>
> **2.3.26 (`tl-feature-compose` — §1 Build sequence uses per-step H3 sections, not a pipe table; new mechanical Transform 0 strips solo-pipe lines + Transform 2a halts on column-count mismatch; closes the "false H2 heading from misparsed table separator" render bug).** User-reported: implementation.md rendered in an external MD viewer with `| Step | Files | Units | Satisfies |` displayed as a huge bold heading — much larger than the actual `## Build sequence` section heading above it. Root cause: the compose LLM occasionally emits a solo `|` line between the intro paragraph and the table header. CommonMark parsers see the subsequent `|---|---|---|---|` separator as an **H2 setext underline** for the pipe-header line, not as a table separator. Even without the solo-pipe artifact, GFM pipe tables with long `Satisfies` cells wrap badly in many external viewers and are the most-reported format regression. Fix: (a) **§1 Build sequence is now per-step `### Step N — <title>` blocks** with three bullets (`Files`, `Units`, `Satisfies`) and a one-line directive — no table for §1; the plan carries the same data as before, just in a form every MD renderer handles cleanly; (b) **Transform 0 in Rule 0c.i** — regex `^\s*\|+\s*$` deletes any line consisting only of pipes and whitespace, unconditionally; (c) **Transform 2a in Rule 0c.i** — for every remaining pipe table (§2 Impacted components, §b Sub-tasks, etc.) count the pipe-cells in the header row vs the separator row; if they differ, HALT with `blocker: table-column-mismatch` and the exact byte range — the LLM's intent for how to align columns is ambiguous, so auto-fix is unsafe; (d) **Rule 0a #11** added to the halt-trigger table; (e) **Rule 0d MC rendering contract** now spells out the new §1 shape verbatim in the compose LLM's system prompt so the FIRST-PASS output doesn't emit a §1 table. Same v2.3.21 pattern: describe the fix mechanically (Transform 0), enforce mechanically (Rule 0a #11 halt), not as prose the LLM might skip.
>
> **2.3.25 (`/dev:commit` — base-branch prompt + local pull; Stage 7 semantic merge non-skippable; Stage 8 hard-gates on Stage 7 invocation trace).** User-reported gap: the agent doing `/dev:commit` reasoned that Stage 7 (semantic context merge) "would be a no-op" and skipped to Stage 7.5 without invoking `tl-semantic-context-merge`. Same class of "spec exists but agent skips" problem as v2.3.21 and v2.3.23. Also related: Stage 2 (Base branch selection) silently read from `project.json` without confirming the target with the user — so a stale `env_branches.dev` would go undetected. Fix: (a) **Stage 2 now prompts** via `AskUserQuestion` for the base branch (default = `project.json` `env_branches.dev` value, `AskUserQuestion` shows it as recommended); user picks or types an override; protected branches (`main`/`master`/`production`) refuse without `--allow-protected-base`; (b) **Stage 2 pulls the base branch locally** (`git fetch <remote> <base>:refs/remotes/<remote>/<base>`) so Stage 7 has a real baseline to merge against; records `base_remote_sha` and `base_pulled_at` in `commit-run.md`; (c) **Stage 7 execution is MANDATORY and VERIFIABLE** — must invoke `tl-semantic-context-merge` via the Skill tool with a real `subagent_id`, must record `tl_semantic_context_merge_invocation:` block in `commit-run.md` with `invoked_at`, `subagent_id`, `base_ref` (must match Stage 2's), `input_units_scanned`, `baseline_units_scanned`, `merged_units`, `conflicts`; zero-change results are LEGITIMATE outcomes, but SKIPPING THE INVOCATION is not; (d) **Stage 8 preconditions become HARD gates** — refuses to push if `stage-2.base_confirmed_by_user` missing, `stage-2.base_pulled_at` missing, `stage-7.tl_semantic_context_merge_invocation.invoked_at` missing, or `stage-7.base_ref` doesn't match `stage-2.base_remote_sha`. Each failure names a specific `blocker:` code so `/dev:commit --resume` can re-run from the gap. This is the same enforcement pattern as v2.3.23's real-service startup requirement — record actual execution evidence, not intent, and gate downstream stages on that evidence.
>
> **2.3.24 (auth-pattern extraction into endpoint units + verbatim inheritance into §8 + auth-alignment check at code write time; closes the Bearer null bug's root cause).** Root cause of the user's Bearer null bug: tl-code-map extracted auth as free prose ("Requires a valid JWT"), tl-feature-compose's §8 Shared contract inherited a generic Bearer template, dev-stack-adaptive-implementation wrote a generic `localStorage.getItem` client-side call — the resulting frontend didn't match the actual backend middleware (Firebase ID token via `verifyFirebaseIdToken()`). Client sent `Bearer null`, server 401'd, "session expired" surfaced when the user was never signed in. Fix — all in existing files, NO new parallel structures: (a) endpoint unit's `## Auth` section (existing) now uses STRUCTURED fields (Token type / Verified by / Client obtains via / Header format / Server extracts / Failure responses / Server prerequisites); (b) new `## Config prerequisites` section per endpoint unit — env vars + services + failure modes if unset; (c) tl-code-map extraction-guide.md gets a stack-agnostic 6-step auth-pattern extraction procedure — walks the middleware chain in ANY stack (Node/Python/Java/.NET/Go/Ruby/Rust) via a shared 6-step algorithm, uses `references/auth-library-registry.md` to map detected library → token type + client acquisition pattern; (d) new `auth-library-registry.md` — 12+ entries (firebase-admin, jsonwebtoken, passport, next-auth, FastAPI OAuth2PasswordBearer, Django auth, DRF Token, Spring Security, ASP.NET Identity, Devise/Warden, gorilla/sessions, golang-jwt) — extensible, no per-stack code hardcoded in the skill; (e) tl-feature-compose Rule 11.3 §8 "How the caller is identified" row now reads VERBATIM from endpoint units' structured `## Auth`; halts with `blocker: endpoint-auth-not-structured` on free-prose auth; halts with `blocker: auth-pattern-inconsistent` if multiple endpoints in a sub-task use different auth mechanisms; (f) dev-stack-adaptive-implementation Rule 2a — auth-pattern alignment check at code write time; before writing any request-emitting code, verify it uses the endpoint's structured `## Auth` `Client obtains via` + `Header format` verbatim; substituting a sibling repo's stale pattern halts; (g) Rule 13 anti-pattern check #9 — regex for `Bearer null` / `Bearer undefined` construction from a nullable source without a null-guard; halts before writing. Same principle for config prerequisites — extracted from endpoint units, flows into Stage 7's real-service startup check + Stage 11's local-runbook.
>
> **2.3.23 (anti-mock hardening — mocks do NOT satisfy integration/E2E tiers; real-service startup enforced at Stage 7; env-var detection tied to actual startup).** Real-run failure: `/dev:build` produced code, tests passed green, dev/acceptance-map marked every AC ✅ pass. Then the developer ran the feature locally, hit the endpoint, got 401 — because the "integration" tests mocked the backend HTTP and never touched the real auth middleware. Backend needed `FIREBASE_SERVICE_ACCOUNT` env var that wasn't set. The plugin's "100% coverage" claim was cosmetic — no test actually validated real end-to-end behavior. v2.3.23 closes this class of bug: (a) **Rule 7.i in `dev-stack-adaptive-implementation`**: a test that mocks HTTP, DB, or auth middleware is a UNIT test regardless of file naming — claiming it as `integration`/`contract`/`concurrency`/`e2e` is a false coverage claim; (b) **Rule 7.ii**: every test file MUST declare its `# tier:` and `# requires:` headers; (c) **Rule 7.iii**: acceptance-map.md rows carry a `mocked: bool` field per evidence entry with schema validation; (d) **Rule 7.iv**: Rule 13's write-time check detects mock library imports; halts if `tier: integration` (or higher) uses `jest.mock` / `vi.mock` / `nock` / `axios-mock-adapter` / `unittest.mock` / `MagicMock`; (e) **Rule 7.v–vi**: before Stage 7 runs integration+ tests, backend MUST actually start (`npm run dev` / `uvicorn` / `go run`) with health-check probe (30s timeout); startup failure = HALT with named blocker; (f) **Stage 7 §7b.i**: pre-start required external services with env-var verification — if `FIREBASE_SERVICE_ACCOUNT` (or any `.env.example` var) is missing, HALT immediately, don't run tests with partial env; (g) **Stage 8 §8c**: two new row states — `❌ mock-contradicts-tier` and `❌ real-service-not-run` — Stage 8 refuses to mark ✅ pass on either; (h) **Stage 11 §3**: env-var detection now uses TRIPLE source — grep + Stage 7's `external_services_started.env_vars_verified` + repo's `.env.example` — with per-var "where to get" hints (Firebase Console, Vault, `openssl rand`, etc.). Credential-shaped vars (`*SECRET*`, `*TOKEN*`, `FIREBASE_SERVICE_ACCOUNT`) always marked `[SET REQUIRED — credential; do NOT commit]`. Closes the "false 100% coverage" cosmetic gap that shipped broken auth code to a user's local test run.
>
> **2.3.22 (`/dev:commit` Stage 9 — 4-step PR-creation fallback ladder; removes `gh` as a hard dependency).** Real-run report: `/dev:commit` halted at Stage 9 because `gh` CLI was not installed on the machine and the user's teammate reported "no token available for the API." Both claims underspecified reality: the user had already authenticated for `git push` (via Windows Git Credential Manager) — that same PAT is usable for the PR API, we just weren't extracting it. v2.3.22 replaces the single `gh pr create` call in §9c with a 4-step fallback ladder: (1) `gh` CLI if installed + authed; (2) extract PAT via `git credential fill` and call `POST /repos/OWNER/REPO/pulls` — piggybacks the exact HTTPS credential that `git push` used; (3) read OAuth token from `gh` config file if `gh` was authed on a previous machine; (4) print a pre-filled compare URL for the user to click (title + body url-encoded from `dev/pr-summary.md`). Each step logs which method succeeded to `dev/commit-run.md` via new `pr_method:` field. Optional §9c.i offers to install `gh` on demand via detected package manager (scoop / winget / brew) with explicit `AskUserQuestion` prompt — never silent. "PR already exists" handled at every ladder step via PATCH-body flow. New `PENDING_USER_BROWSER` state on Step 4 outcome — commit-run.md records `compare_url`, user completes PR in browser, state stays IN_PROGRESS locally until user manually confirms completion.
>
> **2.3.21 (`tl-feature-compose` — mandatory post-compose mechanical fix pass; closes "rules exist but aren't executing" gap).** v2.3.17.1 added the MC rendering contract (Rule 0d for react-markdown + remark-gfm) and the mechanical-halt list (Rule 0a auto-fixes for pipe-run-on tables, unfenced mermaid, missing blank lines). Screenshots from a real run showed the compose STILL emitting pipe-run-on tables — the "rules exist" claim was cosmetic because nothing actually EXECUTED the mechanical fix; the LLM subagent read the rules as documentation, didn't run the auto-fix, and wrote the broken output straight to disk. v2.3.21 adds Rule 0c.i: MANDATORY POST-COMPOSE, PRE-WRITE MECHANICAL FIX PASS. Explicit ordered transformations (split_table_rows, insert_header_separator, ensure_blank_line_surrounds, tag_bare_mermaid_fences, strip_retired_headings) that MUST run on the compose LLM's draft string BEFORE calling `Write`. Reference implementation of split_table_rows() included in the SKILL as executable pseudocode with the exact regex. New workflow step 7 (Mechanical fix pass) inserted between step 6 (Enforce hard rules) and step 8 (Write); step 8 also adds mandatory read-back SHA verify after Write to catch line-ending / encoding corruption. This closes the class of bug where the compose LLM emits a table with all cells on one physical line and the "auto-fix" described in Rule 0a becomes hopeful text nobody executes.
>
> **2.3.20 (`/dev:build` no-commit invariant + parallel fan-out for split-parent targets; `/dev:commit` gains Stage 7.5 gather-and-commit).** Fixes two build-flow bugs surfaced in a real run: (a) `/dev:build` was creating multiple commits per stage (Stage 5-6, Stage 10) which violated the separation-of-concerns model where `/dev:commit` owns the git-write boundary; (b) `/dev:build` on a split-parent target built only one sub-task instead of fanning out to all sub-tasks in parallel, forcing the user to invoke `/dev:build Subtask-N` per sub-task. The v2.3.20 fixes: `/dev:build` top-of-file invariant now says "Never commits, never stages, never pushes" — all source, test, context-unit updates land in the working tree, uncommitted, until `/dev:commit` gathers them. Stage 10 stage-and-commit block removed (was §10g); Stage 10 writes updated context units to the working tree without any git operation. `/dev:build` §3 routing gains a fan-out rule: sub-task target → 1 worker; split-parent target → 1 worker PER sub-task in parallel with `--concurrency=N` cap (default 5); parent-alone target → 1 worker. Per-sub-task isolation on halt. Consolidated report with per-sub-task URLs + read-back + accept-map status. `/dev:commit` gains Stage 7.5 (between Stage 7 semantic-merge and Stage 8 push): gathers the working tree, categorizes files (`source` / `test` / `context-unit` / `dev-local` / `workspace-local`), runs mandatory secrets scan on staged set (halts on `.env*`, `*credentials*`, `*.pem`, etc.), structures commit(s) with default single-commit convention or `--structured` multi-commit. Never `git add -A`, always specific paths from categorization. Never `--amend`, never `--no-verify`. Excludes `dev/local-runbook.md` and all `dev/*-run.md` files by design.
>
> **2.3.19 (`tl-code-map` authors `shared-context/coding-standards.md` when missing — closes the brownfield authoring gap).** Earlier changelogs added the coding-standards.md CONTRACT (12 sections per `tl-project-scaffold` §4) and ENFORCEMENT (`dev-stack-adaptive-implementation` Rule 13 write-time check, Rule 14 hard-precondition, `dev-stack-adaptive-code-review` Dimension 8 review-time check, `/dev:build` Stage 0 hard-gate). BUT the AUTHORING path was only wired for greenfield (via `tl-project-scaffold`). Brownfield workspaces — repos joining Delivery OS via `/jetrix:init` + `/tl:code-map` — had no author for the file, so Rule 14 + Stage 0 hard-blocked every first-time `/dev:build` with `blocker: coding-standards-missing`. This session's fix: promote `tl-code-map` Pass C step 15 from WARN-only to full authoring. The tl-code-map skill has ALREADY detected the stack (Pass A step 1) and read lint config + sample source files (Phase 2 pattern inference) — it's the natural author. §1–§5 (naming, folders, lint, tests, error handling) come from repo scan describing what the code already does; §6–§12 (complexity budget, duplication, recursion, magic values, state, comments, anti-patterns) come from a stack-dynamic resolution — first from repo's own lint/tooling config if declared, then from the stack's ecosystem-default recommended-config values (whatever that stack's standard lint suite recommends when no override is declared). No per-stack numbers hardcoded in the skill instructions — the values come from the STACK'S own lint ecosystem at runtime. Every §6–§12 value logs a DEC-### to `shared-context/decision-log.md` with source attribution (existing config / stack default / observed pattern). File carries full metadata inside — this is a background infrastructure doc, NOT an MC task tab, so `harness_status: Ready | Stack-Inferred`, `stack_inferred`, `existing_config_sources`, `stack_default_sources` all live in the file's frontmatter for downstream flow consumption. `/dev:build` Stage 0 Rule 14 gate remains hard — never softened — because with this step, a properly-mapped brownfield workspace always has the file present. First `/dev:build` on an unmapped workspace still halts with the "run /tl:code-map first" message, which is correct: map the code, then build against it.
>
