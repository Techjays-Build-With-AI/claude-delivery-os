...
When the agent returns, present its **run summary**: sources added/processed (new/changed/unchanged/inaccessible/reference-indexed), summaries generated, scope updates, **open questions raised** and **questions answered/closed this run**, contradictions, assumptions, anything needing user guidance, and recommended next actions. Link to `ba-output/scope.md`, the client-facing `ba-output/client-questions.md`, the run file under `ba-output/intake-runs/`, and `intake.index.md`.
