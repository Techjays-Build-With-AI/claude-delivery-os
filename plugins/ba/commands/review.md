---
description: Review a BA scope document the way a paranoid Business Analyst would before estimate or sign-off. Pass the scope path (defaults to ba/scope.md); the BA agent breaks it down feature by feature, interrogates each for under-specification, validates it against the client's shared examples, scores each feature out of 10, and writes a timestamped interactive scope-review-<timestamp>.html dashboard plus the matching .md and .json to ba/reviews/ (so repeated runs never overwrite each other).
