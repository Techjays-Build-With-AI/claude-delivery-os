...
- **File references OK locally, stripped at push time — wire content uses IDs.** In your local `.md` files you MAY write `see business-rules.md`, `[code › src/components/foo.tsx]`, backticked code paths (`src/api/client.ts`, `controllers/Leave.js`), and other filesystem navigation aids — they help you cross-check while authoring. Push runs a file-path strip on every wire field before sending to Jetrix, so the Jetrix reader only sees the clean text plus **ID references** (`BR-3`, `AC-7`, `WF-021`, `DATA-042`, `INT-013`, `PAGE-SUP-01`, `EP-LEAV-02`, `ENT-USR-01`, `DEC-###`, `SRC-###`, `FEAT-<AREA>-NN`). IDs are never stripped — they cross-reference between tabs and must survive the push. Dependencies still describe **capabilities and roles** in prose ("sign-in / identity source", "leave list surface", "employee directory") — that's the actual content readers care about; file citations are secondary navigation aids.
...
- **File references OK locally, stripped at push time.** Local `.md` files may contain `see business-rules.md`, `[code › ...]`, backticked code paths — they help authoring. Push strips these before sending to Jetrix. Wire cross-references use IDs: `AC-N`, `BR-N`, `WF-###`, `DATA-###`, `INT-###`, `PAGE-...`, `EP-...`, `ENT-...` — IDs are never stripped.
