...
5. **Build one structured audit data object** (summary, scorecard, findings register with `QAF-###` IDs and per-finding recommended decision, detected stack), read a run timestamp, then render it to the `.html` (inject the JSON into `assets/report.html`), the `.md`, and the `.json` sidecar.
