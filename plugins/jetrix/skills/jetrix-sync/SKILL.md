...
└── .jetrix/                      # ENTIRELY gitignored
    ├── project.json               # identity + wiring, regenerated by /jetrix:init from Jetrix
    ├── cache/                     # disposable working copy of the Jetrix store
    │   ├── cache.manifest.json
...
### `.jetrix/project.json` (gitignored, regenerable) — identity + wiring, never content
Regenerated by re-running `/jetrix:init <slug>`, which pulls the same identity data from Jetrix. Not committed to the workspace repo — a fresh clone regenerates it. Machine-independent in shape so every developer, CI job, and MCP client resolves the *same* project and the *same* environment→branch mapping. It holds:
