...
>
> For each targeted feature: read the eight BA-authored files explicitly — `feature.md` (objective / in-scope / out-of-scope + frontmatter identity), `workflow.md` (user flows + mermaid), `acceptance-criteria.md` (`AC-N` rows citing `BR-N`), `business-rules.md` (feature-scoped `BR-1..N` + `mapped_br_ids` frontmatter), `nfrs.md` (essentials-first: concurrency / auditability / etc.), `test-scenarios.md` (three grouped tables), `dependencies.md` (Depends on + Assumptions), and `open-questions.md` (`OQ-<id>` list carrying blockers you must reference when marking a step `[HELD]`). Do NOT read `implementation-plan.md` or `status.md` — they are local-only and irrelevant to compose. Resolve the feature's owned unit files from the three indexes (frontend pages, backend endpoints, database entities) using the feature-cell matching rule the implementation push uses (word-boundary match on comma-separated cells; 2-hop endpoint → entity chain). Read every owned page, endpoint, and entity file — the graph's file paths and framework detail are your INPUT for accuracy, not your output.
>
> Compose the file per `references/implementation-plan-template.md` with exactly these five subsections in this order. Cross-feature "must exist first" waits belong in the **Dependencies tab** (BA-owned, produced from `dependencies.md`) — do NOT restate them here. Code-reuse targets belong in **Touch points** at the end of this document — do NOT create a separate Prerequisites section.
>
> 1. **Build sequence** — one paragraph naming the phases + their dependency order, plus one mermaid step-graph. **No step table.** Exit conditions live inline in the API and Frontend sections below; this diagram is only the sequence map. Steps blocked by undecided OQs are marked `[HELD · waiting on OQ-<id>]` in the paragraph.
...
> 5. **Touch points** — Reuse / New table naming existing and new components **by role**. Every REUSE from the context graph's Reuse Ledger is a row here (never in a separate Prerequisites section). Every NEW component the feature creates is also a row. Include the internal review caveat about re-verifying reuse entries.
>
