...
**Deployment model changes the severity — check it before scoring.** *How* the model is hosted reframes the whole data-use question. A **managed enterprise cloud LLM** — Azure OpenAI / Azure AI Foundry, AWS Bedrock, GCP Vertex — processes prompts inside your own cloud tenant and region under the provider's enterprise **DPA**, and does **not** train on your data by default. That is materially different from a **consumer/standard API key** (e.g. the public OpenAI or Anthropic API) where you must verify the data-use terms yourself. So for a managed deployment, "is our data used to train / has anyone reviewed the terms?" is usually *already answered* by the enterprise agreement, and the real spec gap is narrower:
  - the **data-retention / abuse-logging** decision — e.g. Azure's default abuse-monitoring log (up to ~30 days, human-reviewable), which eligible customers can switch off via Limited Access / *modified abuse monitoring* for effectively zero retention;
  - **region / residency** (which region, does data stay in it);
  - the **legal basis** for any genuine personal data (GDPR etc.) — note that processing personal data needs a basis *even when retention is zero*.

Score that narrower gap — usually **Major or Minor**, a documentation/config item — and **reserve Blocker for data that actually flows somewhere non-compliant with no available fix** (a consumer endpoint with training-on-data left on, regulated PII leaving its allowed region with no DPA, or no mechanism to disable retention). Flagging that the spec is *silent* on retention/residency/legal-basis is still correct; just make the severity and the suggested fix fit the platform — don't hand a team an "OpenAI public API" remediation when they're on Azure under an enterprise DPA.
...
- **Don't over-escalate an easily-configurable control.** If the compliance/ops gap is "the spec doesn't *document* a setting the platform already supports" (e.g. disabling Azure's abuse-monitoring retention, enabling an in-region endpoint), that's a Major/Minor documentation item — not a Blocker. Reserve Blocker for risks with no in-place mechanism to fix. Match the fix to the actual stack: a remediation that names the wrong provider's controls is worse than no remediation, because it sends the team down the wrong path.
