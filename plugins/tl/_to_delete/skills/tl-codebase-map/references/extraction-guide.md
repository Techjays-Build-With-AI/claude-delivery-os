...
`<repo>/code-context/` — the **root of the mapped repository**, committed with the code. Not the workspace, not inside `.jetrix/`. When several repos are mapped from one workspace, each gets its own tree, and `<workspace>/.jetrix/<project-name>/context/code-map-registry.md` points at all of them. Two consequences for extraction:
