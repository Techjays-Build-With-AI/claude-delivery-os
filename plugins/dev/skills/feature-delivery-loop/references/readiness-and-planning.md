...
| A usable test harness exists; quality gates are defined | `qa/quality-gates.md` (`harness_status: Active`) | ✕-critical → **no harness** routes to QA (§1b), not a plain block |
...
1. **Read `qa/quality-gates.md`.** If it exists with `harness_status: Active`, the harness is proven and the file tells `dev-validation` which checks are **Required**, their commands, and the thresholds (coverage floor, when e2e/contract tests are mandatory). Proceed — and carry the required gates into the acceptance-map step.
