...
- **`out=<prefix>`** — optional output prefix override (default `doc/decks/deck-<client>`). A run timestamp is always appended.
...
If a Delivery OS workspace exists (`ba/scope.md` present), tell the agent to auto-draft from it. If not, the agent uses what's provided/attached and marks any missing client numbers as placeholders.
