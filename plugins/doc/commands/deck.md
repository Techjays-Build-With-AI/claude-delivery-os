...
- **`out=<prefix>`** — optional output prefix override (default `doc-output/deck-<title-or-client>`). A run timestamp is always appended.
...
If a Delivery OS workspace exists (`ba-output/scope.md` present), tell the agent it may auto-draft from it. If not, the agent uses the source/what's provided and marks any missing numbers as placeholders.
