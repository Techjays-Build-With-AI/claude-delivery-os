 ...
 *   2. Edit CONFIG (names, date, accent colours if the brand differs).
...
const SANS="Arial", MONO="Roboto Mono";
...
function base(s, dark){
  ...
  s.addText(FOOTER, { x: LX+0.28, y: 7.02, w: 9, h: 0.3, fontFace: MONO, fontSize: 8.5,
    color: dark ? "6E7A93" : FAINT, charSpacing: 0.4, margin: 0 });
  return s;
}
...
function mono(s, o){
  s.addText(o.t, { x:o.x, y:o.y, w:o.w, h:o.h||0.34, fontFace:MONO, fontSize:o.sz||11,
    color:o.c||MUT, bold:o.b||false, align:o.a||"left",
    charSpacing:o.cs!=null?o.cs:0.5, margin:0, valign:o.v||"top" });
}
function topbar(s, label, marker){
  mono(s, { t:label, x:LX, y:0.6, w:8, sz:13, c:INK, cs:0.8 });
  mono(s, { t:marker, x:RX-4, y:0.62, w:4, sz:11, c:FAINT, a:"right" });
}
// small square + single muted line
function bullet(s, x, y, w, text, col){
  s.addShape(p.ShapeType.rect, { x, y:y+0.055, w:0.1, h:0.1, fill:{color:col||C1}, line:{type:"none"} });
  s.addText(text, { x:x+0.26, y, w:w-0.26, h:0.6, fontFace:SANS, fontSize:13.5, color:MUT,
    margin:0, lineSpacingMultiple:1.12, valign:"top" });
}
...
mono(s, { t: CFG.date, x: RX-6, y: 0.6, w: 6, sz: 12, c: MUT, a: "right" });
...
kpis.forEach((k,i) => {
  ...
  mono(s, { t:k[1], x:x+0.24, y:6.12, w:kw-0.24, sz:11, c:MUT, cs:0.3 });
});
...
mono(s, { t: "At a glance — full detail in the appendix", x: LX, y: 2.28, w: 9, sz: 11, c: FAINT, cs: 0.4 });
const mods = [
...
phases.forEach((ph,i) => {
  ...
  mono(s, { t:ph[1], x, y:py+0.72, w:pw, sz:11, c:MUT, cs:0.5 });
  s.addText(ph[2], { x, y:py+1.08, w:pw, h:0.5, fontFace:SANS, fontSize:16.5, bold:true, color:INK, margin:0 });
  s.addText(ph[3], { x, y:py+1.6, w:pw-0.15, h:0.9, fontFace:SANS, fontSize:12.5, color:MUT, margin:0, lineSpacingMultiple:1.15 });
});
mono(s, { t: "The first two weeks need access to the client's codebase.", x: LX, y: 6.35, w: 11, sz: 11, c: FAINT });
...
mono(s, { t: "$75,000 / MONTH   ·   3 MONTHS", x: LX, y: 3.35, w: 6, sz: 13, c: INK, cs: 0.6 });
mono(s, { t: "12 WEEKS   ·   AUDIT → BUILD → GO LIVE", x: LX, y: 3.75, w: 6, sz: 12, c: MUT, cs: 0.6 });
...
mono(s, { t: "HOURS RETURNED / YEAR", x: LX, y: 5.5, w: 6, sz: 11, c: C3, cs: 0.6 });
...
mono(s, { t: "What's Included", x: rx, y: 2.05, w: rw, sz: 13, c: INK, cs: 0.6 });
...
mono(s, { t: "Priced as a single engagement.", x: rx, y: 5.7, w: rw, sz: 11, c: FAINT });
...
s = darkSlide();
mono(s, { t: "END OF CORE DECK   ·   PAGES 1–6", x: LX, y: 2.55, w: 9, sz: 11, c: "7E8BA6", cs: 1 });
...
mono(s, { t: "Module scope   ·   what the client gets   ·   timeline breakdown", x: LX, y: 5.65, w: 11, sz: 12, c: "AEB8CC", cs: 0.4 });
...
detail.forEach(d => {
  ...
  mono(s, { t:"WHAT WE BUILD", x:LX, y:2.5, w:cw2, sz:11, c:d.col, b:true, cs:0.8 });
  let yy=3.0; d.build.forEach(b => { drow(s, LX, yy, cw2, b[0], b[1], d.col); yy += (b[1]&&b[1].length>40)?0.82:0.62; });
  if (d.note) mono(s, { t:d.note, x:LX, y:6.5, w:cw2, sz:9.5, c:FAINT });
  mono(s, { t:"WHAT THE CLIENT GETS", x:x2, y:2.5, w:cw2, sz:11, c:d.col, b:true, cs:0.8 });
...
for (let k=0;k<WEEKS;k++){
  ...
  mono(s, { t:String(k+1), x:cx, y:gy0-0.42, w:colW, sz:9.5, c:FAINT, a:"center", cs:0 });
}
mono(s, { t:"WEEK", x:gx-1.0, y:gy0-0.42, w:0.9, sz:9, c:FAINT, a:"right" });
...
mono(s, { t: "Let's build it together.", x: LX, y: 5.15, w: 10, sz: 14, c: "AEB8CC", cs: 0.6 });
